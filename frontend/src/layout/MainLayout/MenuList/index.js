...
const MenuList = () => {
    ...
    const handlerMenuItem = () => {
        const filteredItems = menuItem.items.filter((item) => item.id !== 'widget');

        if (getMenu?.id !== 'widget' && !filteredItems.some((item) => item.id === getMenu.id)) {
            filteredItems.splice(1, 0, getMenu);
        }
